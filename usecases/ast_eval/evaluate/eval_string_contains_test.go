...
func TestString_Contains_wrong_number_of_arguments(t *testing.T) {
	_, errs := evaluate.NewStringContains(ast.FUNC_STRING_CONTAINS).Evaluate(context.TODO(), ast.Arguments{Args: []any{"abc"}})
	assert.NotEmpty(t, errs)
	assert.ErrorIs(t, errs[0], ast.ErrWrongNumberOfArgument)
}

func TestString_Contains_wrong_type_of_arguments(t *testing.T) {
	_, errs := evaluate.NewStringContains(ast.FUNC_STRING_CONTAINS).Evaluate(context.TODO(), ast.Arguments{Args: []any{"abc", 1}})
	assert.NotEmpty(t, errs)
