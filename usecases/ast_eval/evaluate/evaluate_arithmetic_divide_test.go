...
func TestNewArithmetic_divide_float64(t *testing.T) {
	...
	r, errs := ArithmeticDivide{}.Evaluate(ast.Arguments{Args: []any{10.0, 3}})
	assert.Empty(t, errs)
...
func TestNewArithmetic_divide_int(t *testing.T) {
	...
	r, errs := ArithmeticDivide{}.Evaluate(ast.Arguments{Args: []any{10, 3}})
	assert.Empty(t, errs)
	assert.Equal(t, r, TEN_DIVIDE_BY_THREE)
}

func TestNewArithmeticFunction_float_divide_by_zero(t *testing.T) {
	_, errs := ArithmeticDivide{}.Evaluate(ast.Arguments{Args: []any{1.0, 0.0}})
