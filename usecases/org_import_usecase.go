...
func (uc *OrgImportUsecase) Import(ctx context.Context, spec dto.OrgImport, seed bool) (uuid.UUID, error) {
	...
	return executor_factory.TransactionReturnValue(ctx, uc.transactionFactory, func(tx repositories.Transaction) (uuid.UUID, error) {
		...
		if seed {
			if err := uc.Seed(ctx, spec, orgId.String()); err != nil {
				return orgId, nil
			}
		}
...
func (uc *OrgImportUsecase) createOrganization(ctx context.Context, tx repositories.Transaction, spec dto.OrgImport) (uuid.UUID, error) {
	...
	if err := uc.orgRepository.CreateOrganization(ctx, tx, orgId.String(), spec.Org.Name); err != nil {
		return uuid.Nil, err
	}

	org, err := uc.orgRepository.GetOrganizationById(ctx, tx, orgId.String())
	if err != nil {
		return uuid.Nil, err
	}
...
func (uc *OrgImportUsecase) createDataModel(ctx context.Context, tx repositories.Transaction, clientDbExec repositories.Executor, orgId uuid.UUID, ids map[string]string, dataModel dto.ImportDataModel) error {
	...
	for _, table := range dataModel.Tables {
		...
		if err := uc.dataModelRepository.CreateDataModelTable(ctx, tx, orgId.String(), tableId.String(), table.Name, table.Description /* TODO */, nil); err != nil {
			return err
		}
		...
		for name, field := range table.Fields {
			...
			if err := uc.dataModelRepository.CreateDataModelField(ctx, tx, orgId.String(), fieldId.String(), field); err != nil {
				return err
			}
		}
	}
...
func (uc *OrgImportUsecase) createScenarios(ctx context.Context, tx repositories.Transaction, orgId uuid.UUID, ids map[string]string, scenarios []dto.ImportScenario) error {
	for _, scenario := range scenarios {
		...
		err := uc.scenarioRepository.CreateScenario(ctx, tx, orgId.String(), models.CreateScenarioInput{
			OrganizationId:    orgId.String(),
		...
		iteration, err := uc.iterationRepository.CreateScenarioIterationAndRules(ctx, tx, orgId.String(), models.CreateScenarioIterationInput{
		...
		indexes, pending, err := uc.indexEditor.GetIndexesToCreate(ctx, orgId.String(), iteration.Id)
		if err != nil {
			return err
		}

		if len(indexes) > 0 || pending > 0 {
			if err := uc.indexEditor.CreateIndexesBlocking(ctx, orgId.String(), indexes); err != nil {
				return err
			}
		}

		_, err = uc.publicationUsecase.ExecuteScenarioPublicationAction(ctx, orgId.String(), models.PublishScenarioIterationInput{
