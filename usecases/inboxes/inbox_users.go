...
func (usecase *InboxUsers) ListAllInboxUsers(ctx context.Context) ([]models.InboxUser, error) {
	if !usecase.Credentials.HasRole(models.ADMIN) && !usecase.Credentials.HasRole(models.MARBLE_ADMIN) {
...
func (usecase *InboxUsers) UpdateInboxUser(ctx context.Context, inboxUserId uuid.UUID,
	...
	inboxUser, err := executor_factory.TransactionReturnValue(
		...
		func(tx repositories.Transaction) (models.InboxUser, error) {
			...
			if err := usecase.InboxUserRepository.UpdateInboxUser(ctx, tx, inboxUserId, role, autoAssignable); err != nil {
				return models.InboxUser{}, err
			}
