...
func (v MarbleVerifier) Verify(ctx context.Context, creds Credentials) (models.IntoCredentials, models.IdentityClaims, error) {
	...
	// An API key is already enough to retrieve the credentials in the generator
	case CredentialsApiKey:
		...
		apiKey.Representation = fmt.Sprintf("Api key %s*** of %s", apiKey.Prefix, organization.Name)
