...
type CaseUseCaseRepository interface {
	...
	GetCasesWithObjectAttr(ctx context.Context, exec repositories.Executor,
...
func (usecase *CaseUseCase) GetRelatedCasesByObjectAttr(
	...
	cases, err := usecase.repository.GetCasesWithObjectAttr(ctx, exec, orgId, objectType, objectId)
	if err != nil {
		return nil, err
	}
