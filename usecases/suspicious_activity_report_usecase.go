...
func (uc SuspiciousActivityReportUsecase) ListReports(
	...
	exec := uc.caseUsecase.executorFactory.NewExecutor()
...
func (uc SuspiciousActivityReportUsecase) GenerateReportUrl(
	...
	return uc.caseUsecase.blobRepository.GenerateSignedUrl(ctx, *sar.Bucket, *sar.BlobKey)
}
...
func (uc SuspiciousActivityReportUsecase) UploadReport(
	...
	sar, err := executor_factory.TransactionReturnValue(ctx, uc.caseUsecase.transactionFactory, func(
		...
		if err := writeToBlobStorage(ctx, uc.caseUsecase, req.File, blobKey); err != nil {
			return models.SuspiciousActivityReport{}, err
		}

		req.Bucket = uc.caseUsecase.caseManagerBucketUrl
		req.BlobKey = blobKey
...
func (uc SuspiciousActivityReportUsecase) canUpdateCase(ctx context.Context,
	...
	if err := uc.caseUsecase.enforceSecurity.ReadOrUpdateCase(c, inboxIds); err != nil {
		return models.Case{}, err
	}
