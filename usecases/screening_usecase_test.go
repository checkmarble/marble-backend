...
func TestListScreeningOnDecision(t *testing.T) {
	...
	exec.Mock.ExpectQuery(escapeSql(`
		SELECT
			sc.id, sc.decision_id, sc.org_id, sc.sanction_check_config_id, sc.status, sc.search_input, sc.initial_query, sc.search_datasets, sc.match_threshold, sc.match_limit, sc.is_manual, sc.requested_by, sc.is_partial, sc.is_archived, sc.initial_has_matches, sc.whitelisted_entities, sc.error_codes, sc.created_at, sc.updated_at,
...
func TestUpdateMatchStatus(t *testing.T) {
	...
	exec.Mock.
		ExpectQuery(`SELECT id, decision_id, org_id, sanction_check_config_id, status, search_input, initial_query, search_datasets, match_threshold, match_limit, is_manual, requested_by, is_partial, is_archived, initial_has_matches, whitelisted_entities, error_codes, created_at, updated_at FROM sanction_checks WHERE id = \$1`).
