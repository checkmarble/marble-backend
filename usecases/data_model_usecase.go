...
func (usecase DataModelUseCase) GetDataModel(
	...
	dataModel, err := usecase.dataModelRepository.GetDataModel(ctx, exec, organizationID, options.IncludeEnums, true)
	if err != nil {
		return models.DataModel{}, err
	}

	if options.IncludeNavigationOptions {
		pivotsMeta, err := usecase.dataModelRepository.ListPivots(ctx, exec, organizationID, nil, false)
		if err != nil {
			return models.DataModel{}, err
		}
