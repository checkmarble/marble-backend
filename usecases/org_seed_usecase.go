...
func (uc *OrgImportUsecase) Seed(ctx context.Context, spec dto.OrgImport, orgId string) error {
	ids := make(map[string][]string, 0)
	tableMap := make(map[string]dto.ImportSeedsIngestion)

	for _, tableSpec := range spec.Seeds.Ingestion {
		...
		for range tableSpec.Count {
			...
			if _, err := uc.ingestionUsecase.IngestObject(ctx, orgId, tableSpec.Table, object); err != nil {
				return err
			}
		}
	}
