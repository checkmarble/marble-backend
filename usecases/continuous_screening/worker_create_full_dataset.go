	...
	err = w.transactionFactory.Transaction(ctx, func(tx repositories.Transaction) error {
		...
		datasetFile, err := w.repo.CreateContinuousScreeningDatasetFile(ctx, tx,
			models.CreateContinuousScreeningDatasetFile{
				...
				Status:   models.ContinuousScreeningDatasetFileStatusUploaded,
			})
		if err != nil {
...
// Version format: "YYYYMMDD-XXX" where XXX is a zero-padded index (e.g., "20251230-001").
// If the previous version's date matches today's date, the index is incremented.
// Otherwise, the index resets to 001.
func generateNextVersion(previousVersion string, now time.Time) string {
	todayDate := now.Format("20060102")

	if previousVersion == "" {
		return fmt.Sprintf("%s-001", todayDate)
	}
	...
	prevDate := parts[0]
	prevIndexStr := parts[1]

	if prevDate == todayDate {
		// Same day, increment the index
		prevIndex, err := strconv.Atoi(prevIndexStr)
		if err != nil {
			return fmt.Sprintf("%s-001", todayDate)
		}
		return fmt.Sprintf("%s-%03d", todayDate, prevIndex+1)
	}

	// Different day, reset to 001
	return fmt.Sprintf("%s-001", todayDate)
}
