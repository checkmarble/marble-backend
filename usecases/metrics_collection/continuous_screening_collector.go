...
type ContinuousScreeningCollectorRepository interface {
	CountCSScreeningsByProvider(ctx context.Context, exec repositories.Executor, orgIds []string, providers []string,
		from, to time.Time,
	...
	for _, period := range periods {
		...
		for orgId, orgCount := range orgCSScreeningCounts {
			for provider, providerCount := range orgCount {
				screeningCountMetricName, err := buildCSScreeningMetricName(provider)
				if err != nil {
					// Should never happen
					return nil, err
				}
