...
resource "google_cloud_scheduler_job" "scheduler_cron" {
  name             = "scheduler-cron"
  schedule         = "10 * * * *"
  time_zone        = "Etc/UTC"
  ...
  http_target {
    http_method = "POST"
    uri         = "https://${local.location}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${local.project_id}/jobs/${google_cloud_run_v2_job.scheduler.name}:run"

    oauth_token {
      service_account_email = google_service_account.backend_service_account.email
    }
  }
  depends_on = [google_cloud_run_v2_job.scheduler, google_project_iam_member.backend_service_account_cron_run_invoker]
}
