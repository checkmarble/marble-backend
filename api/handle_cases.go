...
func handleGetRelatedCasesByObjectAttr(uc usecases.Usecases) func(c *gin.Context) {
	return func(c *gin.Context) {
		ctx := c.Request.Context()
		creds, found := utils.CredentialsFromCtx(ctx)
		if !found {
		...
		uc := usecasesWithCreds(ctx, uc).NewCaseUseCase()
		cases, err := uc.GetRelatedCasesByObjectAttr(ctx, creds.OrganizationId, objectType, objectId)
		if presentError(ctx, c, err) {
			return
		}
