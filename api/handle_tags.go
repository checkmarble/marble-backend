...
func (api *API) handleListTagsPerInbox(ctx *gin.Context) {
	var inboxInput InboxIdUriInput
	if err := ctx.ShouldBindUri(&inboxInput); err != nil {
		ctx.Status(http.StatusBadRequest)
		return
	}
	usecase := api.UsecasesWithCreds(ctx.Request).NewTagUseCase()
	tags, err := usecase.ListInboxTags(inboxInput.InboxId)
