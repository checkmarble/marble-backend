...
func addRoutes(r *gin.Engine, conf Configuration, uc usecases.Usecases, auth utils.Authentication, tokenHandler TokenHandler, logger *slog.Logger) {
	...
	router.GET("/client_data/:object_type/:object_id/cases", tom, handleEntityRelatedCases(uc))
	...
	router.POST("/scenarios/:scenario_id/copy", tom, copyScenario(uc))
	router.POST("/scenarios/:scenario_id/validate-ast", tom, validateScenarioAst(uc))
	router.POST(
		"/scenarios/:scenario_id/ast-ai-description", timeoutMiddleware(conf.BatchTimeout),
	...
	router.POST("/screenings/searches", tom, handleSaveScreeningManualSearch(uc))
	router.GET("/screenings/searches", tom, handleListScreeningManualSearch(uc))
	router.DELETE("/screenings/searches/:id", tom, handleDeleteScreeningSavedSearch(uc))
	...
	router.PATCH(
		"/continuous-screenings/configs/:stable_id", tom,
	...
	router.GET("/continuous-screenings/client-data-indexing", tom,
		handleListContinuousScreeningClientDataIndexing(uc))
	...
	router.GET("/cases/:case_id/reviews", tom, handleListCaseReviews(uc))
	router.GET("/cases/:case_id/review/:review_id", tom, handleGetCaseReviewById(uc))
	...
	router.POST(
		"/org-import",
		timeoutMiddleware(conf.BatchTimeout),
		limits.RequestSizeLimiter(maxCaseFileSize),
		handleOrgImport(uc),
	)
	router.POST(
		"/org-import/file",
