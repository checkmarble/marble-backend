...
func setupApiCreds(ctx context.Context, t *testing.T, usecasesWithCreds usecases.UsecasesWithCreds, organizationId uuid.UUID) models.Credentials {
	...
	apiKey, err := apiKeyUsecase.CreateApiKey(ctx, models.CreateApiKeyInput{
		...
		RoleBindings:   models.NativeRoleBindings([]models.Role{models.API_CLIENT}),
	})
	if err != nil {
	...
	intoCredentials, claims, err := apiKeyVerifier.Verify(ctx, in)
	if err != nil {
...
func createDecisions(
	...
	_, err = ruleSnoozeUsecase.SnoozeDecision(ctx, models.SnoozeDecisionInput{
		...
		Duration:       "5s", // snooze for 5 seconds, after this wait for the snooze to end before moving on
		OrganizationId: organizationId,
