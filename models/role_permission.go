package models

var VIEWER_PERMISSIONS = []Permission{
	ANALYTICS_READ,
	DECISION_READ,
	SCENARIO_READ,
	DATA_MODEL_READ,
	CUSTOM_LISTS_READ,
	CASE_READ_WRITE,
	SCREENING_WHITELIST_READ,
	SCREENING_WHITELIST_WRITE,
	WEBHOOK_EVENT,
	READ_SNOOZES,
	CREATE_SNOOZE,
	TAG_READ,
	MARBLE_USER_READ,
	MARBLE_USER_LIST,
	MARBLE_USER_UPDATE,
	CONTINUOUS_SCREENING_CONFIG_READ,
	CONTINUOUS_SCREENING_CONFIG_WRITE,
	CONTINUOUS_SCREENING_HIT_READ,
	CONTINUOUS_SCREENING_HIT_WRITE,
	CONTINUOUS_SCREENING_OBJECT_READ,
	CONTINUOUS_SCREENING_OBJECT_WRITE,
	SCREENING_FREEFORM_SEARCH,
}

var (
	BUILDER_PERMISSIONS   = append(VIEWER_PERMISSIONS, SCENARIO_CREATE, CUSTOM_LISTS_EDIT)
	PUBLISHER_PERMISSIONS = append(BUILDER_PERMISSIONS, SCENARIO_PUBLISH, SCORING_UPDATE_RULESETS)
	ADMIN_PERMISSIONS     = append(
		PUBLISHER_PERMISSIONS,
		APIKEY_READ,
		APIKEY_CREATE,
		DATA_MODEL_WRITE,
		DECISION_CREATE, // Used to validate permission to start a scheduled execution of a scenario
		INGESTION,
		MARBLE_USER_CREATE,
		MARBLE_USER_DELETE,
		INBOX_EDITOR,
		WEBHOOK,
		TAG_CREATE,
		TAG_UPDATE,
		TAG_DELETE,
		ORGANIZATIONS_UPDATE,
		ANNOTATION_DELETE,
		ANNOTATION_RISK_TAG_WRITE,
		CONTINUOUS_SCREENING_HIT_DISMISS, // Only admin can dismiss all hits for a continuous screening
		ORG_IMPORT_ARCHETYPE_READ,        // Only org admin can list organization archetypes
		ORG_IMPORT_INTO_EXISTING,         // Only org admin can seed an organization via import
		ORG_EXPORT,
		SCORING_UPDATE_SETTINGS,
		SCORING_OVERRIDE_SCORE,
	)
	TENANT_ADMIN_PERMISSIONS = []Permission{}
)

var ROLES_PERMISSIONS = map[Role][]Permission{
	NO_ROLE:   {},
	VIEWER:    VIEWER_PERMISSIONS,
	ANALYST:   VIEWER_PERMISSIONS,
	BUILDER:   BUILDER_PERMISSIONS,
	PUBLISHER: PUBLISHER_PERMISSIONS,
	ADMIN:     ADMIN_PERMISSIONS,
	API_CLIENT: {
		SCENARIO_READ, // Used to validate the scenario being executed in the decision usecase
		DECISION_READ,
		DECISION_CREATE,
		PHANTOM_DECISION_CREATE, // Necessary because a permission check is embedded in the phantom decision usecase
		INGESTION,
		WEBHOOK_EVENT, // Necessary because a permission check is embedded in the webhooks events usecase
		CASE_READ_WRITE,
		READ_SNOOZES,
		CREATE_SNOOZE,
		SCREENING_WHITELIST_READ,
		SCREENING_WHITELIST_WRITE,
		SCREENING_FREEFORM_SEARCH,
		// Permissions required for the case public API
		MARBLE_USER_LIST,
		MARBLE_USER_READ,
		INBOX_EDITOR,
		TAG_READ,
		CONTINUOUS_SCREENING_CONFIG_READ,
		CONTINUOUS_SCREENING_CONFIG_WRITE,
		CONTINUOUS_SCREENING_HIT_READ,
		CONTINUOUS_SCREENING_HIT_WRITE,
		CONTINUOUS_SCREENING_OBJECT_READ,
		CONTINUOUS_SCREENING_OBJECT_WRITE,
		ANNOTATION_RISK_TAG_WRITE,
		ANNOTATION_DELETE,
		SCORING_OVERRIDE_SCORE,
	},
	MARBLE_ADMIN: append(
		ADMIN_PERMISSIONS,
		PHANTOM_DECISION_CREATE,
		ORGANIZATIONS_LIST,
		ORGANIZATIONS_CREATE,
		ORGANIZATIONS_UPDATE,
		ORGANIZATIONS_DELETE,
		ANY_ORGANIZATION_ID_IN_CONTEXT,
		LICENSE_LIST,
		LICENSE_CREATE,
		LICENSE_UPDATE,
	),
	TENANT_ADMIN: TENANT_ADMIN_PERMISSIONS,
}
