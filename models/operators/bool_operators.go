...
// ///////////////////////////////////////////////////////////////////////////////////////
// Trigger object field Boolean
// ///////////////////////////////////////////////////////////////////////////////////////
type TriggerObjectFieldBool struct {
	FieldName string
}

// register creation
func init() {
	operatorFromType["TRIGGER_OBJECT_FIELD_BOOL"] = func() Operator { return &TriggerObjectFieldBool{} }
}

func (field TriggerObjectFieldBool) Eval(ctx context.Context, d DataAccessor) (bool, error) {
	if !field.IsValid() {
		return false, ErrEvaluatingInvalidOperator
	}
	...
	case "batch":
		{
			...
			valRaw, err := d.GetDbField(ctx, d.GetTriggerObjectName(), []string{}, field.FieldName)
			if err != nil {
				return false, err
			}
	...
	default:
		{
			return false, fmt.Errorf("unknown execution type %s", d.ExecutionType())
		}
	}
}

func (field TriggerObjectFieldBool) IsValid() bool {
	return field.FieldName != ""
}

func (field TriggerObjectFieldBool) String() string {
	return fmt.Sprintf("( Boolean field from trigger object: %s )", field.FieldName)
}

func (field TriggerObjectFieldBool) MarshalJSON() ([]byte, error) {

	// data schema
	type triggerObjectFieldBoolData struct {
		FieldName string `json:"fieldName"`
	}

	return json.Marshal(struct {
		OperatorType
		StaticData triggerObjectFieldBoolData `json:"staticData"`
	}{
		OperatorType: OperatorType{Type: "TRIGGER_OBJECT_FIELD_BOOL"},
		StaticData: triggerObjectFieldBoolData{
			FieldName: field.FieldName,
		},
	})
}

func (field *TriggerObjectFieldBool) UnmarshalJSON(b []byte) error {
	// data schema
	var triggerObjectFieldData struct {
		StaticData struct {
			FieldName string `json:"fieldName"`
		} `json:"staticData"`
	}

	if err := json.Unmarshal(b, &triggerObjectFieldData); err != nil {
		return fmt.Errorf("unable to unmarshal operator to intermediate staticData representation: %w", err)
	}
	field.FieldName = triggerObjectFieldData.StaticData.FieldName
