...
create table continuous_screening_dataset_files (
    ...
    status text not null constraint dataset_files_status_check check (status in ('pending', 'processing', 'uploaded', 'failed')),
...
create index if not exists idx_continuous_screening_dataset_files_org_type_status on continuous_screening_dataset_files (org_id, file_type, status);
