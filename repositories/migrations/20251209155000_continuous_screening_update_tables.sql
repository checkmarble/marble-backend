...
create table continuous_screening_update_jobs (
    id uuid primary key default uuid_generate_v4(),
    cs_dataset_update_id uuid not null,
    cs_config_id uuid not null,
    ...
    constraint fk_cs_dataset_update foreign key (cs_dataset_update_id) references continuous_screening_dataset_updates (id) on delete cascade,
    constraint fk_cs_config foreign key (cs_config_id) references continuous_screening_configs (id) on delete cascade,
...
create table continuous_screening_job_offsets (
    id uuid primary key default uuid_generate_v4(),
    cs_update_job_id uuid not null,
    "offset" bigint not null,
    line_processed integer not null,
    total_lines integer not null,
    created_at timestamp with time zone not null default current_timestamp,
    updated_at timestamp with time zone not null default current_timestamp,

    constraint fk_cs_update_job foreign key (cs_update_job_id) references continuous_screening_update_jobs (id) on delete cascade
);
