...
func injectDbSessionConfig(ctx context.Context, exec TransactionOrPool, query string) (pgconn.CommandTag, error) {
	...
	if creds, ok := utils.CredentialsFromCtx(ctx); ok {
		...
		case creds.ActorIdentity.UserId != "":
			cmds = append(cmds, auditCommands{"SET_CONFIG($1, $2, false)", []any{postgres_audit_user_id_parameter, creds.ActorIdentity.UserId}})
		case creds.ActorIdentity.ApiKeyId != "":
			cmds = append(cmds, auditCommands{"SET_CONFIG($1, $2, false)", []any{postgres_audit_api_key_id_parameter, creds.ActorIdentity.ApiKeyId}})
		default:
		...
		if creds.OrganizationId != "" {
			cmds = append(cmds, auditCommands{"SET_CONFIG($3, $4, false)", []any{postgres_audit_org_id_parameter, creds.OrganizationId}})
		}
	}
